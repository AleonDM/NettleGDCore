...
if($_POST["userName"] != ""){
	...
	if ($regusrs > 0) {
		...
		$passwords = file("common-passwords.txt");
		foreach($passwords as $passw) {
			if(trim($passw) == $password) exit("-5"); 
		}
		// cleaning non-registered accounts
		$query = $db->prepare("DELETE FROM accounts WHERE isActive = 0 AND registerDate <= :date");
		$query->execute([":date" => time()-86400]);
